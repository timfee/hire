...
export default function generateIntroduction(
  doc: PDFKit.PDFDocument,
  company: Company
) {
  const WHITESPACE = {
    x: doc.page.margins.left,
    w: doc.page.width - 2 * doc.page.margins.left,
    y: 215,
    h: 500,
    innerPadding: 50,
  }

  bgcolor(doc, { color: colors.slate['600'] }).moveDown(3)
  format
    .name(doc, colors.white)
    .text(resume.basics.name, { characterSpacing: -1.5, features: OT_FEATURES })

  doc
    .rect(WHITESPACE.x, WHITESPACE.y, WHITESPACE.w, WHITESPACE.h)
    .fill(colors.white)
  ...
  format
    ...
    .text(
      `Hi ${company.name}!\n\n`,
      WHITESPACE.x + WHITESPACE.innerPadding,
      WHITESPACE.y + WHITESPACE.innerPadding,
      {
        characterSpacing: -1,
  ...
  format.paragraph(doc).text(company.resumeMessage, {
  ...
  format
    .paragraph(doc)
    .text(
      '\nI’ve put together a dedicated portal with links to my resume, references and more:\n\n',
      {
        characterSpacing: -0.5,
        features: OT_FEATURES,
      }
  ...
  format.paragraph(doc).text('\nThanks for your consideration,\n\n', {
    characterSpacing: -0.5,
    features: OT_FEATURES,
  })

  doc
    .translate(WHITESPACE.x + WHITESPACE.innerPadding, doc.y)
    .scale(0.1, 0.1)
    .path(SIGNATURE_PATH)
