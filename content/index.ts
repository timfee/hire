...
import generateEnding from './blocks/ending'
import generateExperience from './blocks/experience'
...
const generateResumePacket = async ({ company }: { company: Company }) => {
  ...
  doc.registerFont(
    'standard',
    resolve(cwd(), 'styles', 'fonts', 'sans-regular.ttf')
  )
  doc.registerFont(
    'medium',
    resolve(cwd(), 'styles', 'fonts', 'sans-medium.ttf')
  )
  doc.registerFont('bold', resolve(cwd(), 'styles', 'fonts', 'sans-bold.ttf'))
  doc.registerFont('black', resolve(cwd(), 'styles', 'fonts', 'sans-black.ttf'))

  generateIntroduction(doc, company)
  generateHeader(doc)
  generateExperience(doc)
  generateEducation(doc)
  await generateEnding(doc, company)
  addPagination(doc)
...
export const refreshResumeUrl = async (company: Company, force = false) => {
  ...
  return `${env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/hire-timfeeley/${key}`.replace(
    /\s/g,
    '%20'
  )
}
